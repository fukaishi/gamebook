...
1. `admin.html` を開く
2. 既存の `story.json` が自動的に読み込まれる
3. 左側のシーン一覧からシーンを選択
4. 右側のエディタで編集
5. 「シーンを保存」で変更を保存（自動的にブラウザに保存される）
6. 「プレビュー」をクリックして動作確認
7. （オプション）「JSONを保存」で編集したJSONをダウンロード

**重要**:
- 編集内容はブラウザのLocalStorageに自動保存されます
- プレビューは保存された編集内容を反映します
- LocalStorageはブラウザごとに管理されるため、別のブラウザでは編集内容は見えません
- 編集を破棄したい場合はブラウザのLocalStorageをクリアしてください
